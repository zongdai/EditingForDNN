...
from models.partrcnn_transform import PartRCNNTransform
...
class FasterRCNN(GeneralizedRCNN):
    ...
    def __init__(self, backbone, backbone2, is_double_backbone=True, num_classes=None, num_states=6,
        ...
        # states
        states_roi_pool = MultiScaleRoIAlign(
                featmap_names=[0, 1, 2, 3],
                output_size=7,
                sampling_ratio=2)
        resolution = states_roi_pool.output_size[0]
        representation_size = 1024
        states_head = TwoMLPHead(
                out_channels * resolution ** 2,
                representation_size)
        ...
        roi_heads = RoIHeads(
            ...
            bbox_reg_weights,
            box_score_thresh, box_nms_thresh, box_detections_per_img, states_roi_pool=states_roi_pool, states_head=states_head, states_predictor=states_predictor)

        if image_mean is None:
            image_mean = [0.485, 0.456, 0.406]
        if image_std is None:
            image_std = [0.229, 0.224, 0.225]
        transform = PartRCNNTransform(min_size, max_size, image_mean, image_std)

        super(FasterRCNN, self).__init__(backbone, backbone2, rpn, roi_heads, transform, is_double_backbone = is_double_backbone)
...
class StatesRCNNPredictor(nn.Module):
    def __init__(self, in_channels, num_states):
        super(StatesRCNNPredictor, self).__init__()
        self.states_vector = nn.Linear(in_channels, num_states)
    
    def forward(self, x):
        if x.ndimension() == 4:
            assert list(x.shape[2:]) == [1, 1]
        x = x.flatten(start_dim=1)
        states_vector = self.states_vector(x)
